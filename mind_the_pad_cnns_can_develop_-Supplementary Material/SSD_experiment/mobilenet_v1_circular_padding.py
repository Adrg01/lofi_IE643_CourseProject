...
def circular_padding(x):
    padding_top = x[:, -1:, :, :]
    padding_bottom = x[:, 0:1, :, :]
    x = tf.concat([padding_top, x, padding_top], 1, name = "CircPadRows")
    padding_left = x[:, :, -1:, :]
    padding_right = x[:, :, 0:1, :]
    x = tf.concat([padding_left, x, padding_right], 2, name = "CircPadCols")
    return x
...
def mobilenet_v1_base(inputs,
  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding=padding):
      ...
      rate = 1
      cropped_inp = inputs[:, :705, :, :]
      net = tf.concat((cropped_inp, cropped_inp[:, :, -1:, :]), 2)
